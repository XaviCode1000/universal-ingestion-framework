...
), the CLI automatically creates a "checkpoint." This checkpoint
includes:
...
It does If you want to undo the change or simply go back, you can use the `/restore`
...
Note:The`--checkpointing`

command-line flag was removed in version 0.11.0. Checkpointing can now only be enabled through the`settings.json`
...
commandOnce enabled, checkpoints are created automatically. To manage them, you use the
`/restore`
