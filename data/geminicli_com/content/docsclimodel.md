...
Select your Gemini CLI model. The `/model`

command lets you configure the model
...
flag) does not override the model used by sub-agents. Consequently, even when using the`/model`

flag you may see other models used in your model usage reports.

`/model`

commandUse the following command in Gemini CLI:
...
Note:Gemini 3 is not currently available on all account types. To learn more about Gemini 3 access, refer to Gemini 3 on Gemini CLI.

To enable Gemini 3 Pro and Gemini 3 Flash (if available), enable
**Preview Features** by using the `settings`

command.

You can also use the `--model`

flag to specify a particular Gemini model on
