---
url: https://geminicli.com/docs/cli/commands
title: CLI commands
author: null
date: null
sitename: Gemini CLI
...
will become the headline for the bug being filed. The default
`/bug`

behavior can be modified using the `advanced.bugCommand`

setting in
your `.gemini/settings.json`
...
or JSON file.
**Usage** `/chat share file.md`

or `/chat share file.json`
...
. You can typically install them
using your system's package manager.
- On macOS, it requires
`pbcopy`

, and on Windows, it requires `clip`
...
:
**Description:** Display all directories added by `/directory add`
...
**Details:** If `<server-name>`

is provided, it initiates the OAuth flow
...
hierarchical memory that has been loaded from all `GEMINI.md`
...
files found in the configured locations (global,
project/ancestors, and sub-directories). This command updates the model
with the latest `GEMINI.md`
...
**Note:** For more details on how `GEMINI.md`
...
settings. See
Checkpointing documentation for more details.
...
to search through conversation content across all
sessions
...
saving required. See Session Management for
complete details.
...
equivalent to manually editing the `.gemini/settings.json`

file, but with
validation and guidance to prevent errors. See the
settings documentation for a full list of available
settings.
**Usage:** Simply run `/settings`

and the editor will open. You can then
...
**Editing commands:** Delete with `x`
...
**Count support:** Prefix commands with numbers (e.g., `3h`
...
**Description:** To help users easily create a `GEMINI.md`
...
please see the dedicated Custom Commands documentation.
...
is not found or is invalid, an error message
will be displayed, and the query might not be sent to the Gemini model, or it
will be sent without the file content.
- If the
`read_many_files`

tool encounters an error (e.g., permission issues),
this will also be reported.

The `!`

prefix lets you interact with your system's shell directly from within
Gemini CLI.

-
`!<shell_command>`

**Description:** Execute the given `<shell_command>`

using `bash`

on
Linux/macOS or `powershell.exe -NoProfile -Command`

on Windows (unless you
override `ComSpec`

). Any output or errors from the command are displayed in
the terminal.
**Examples:**
`!ls -la`
...
usage: Commands you execute in shell mode have the
...
environment variable is set in the subprocess's
